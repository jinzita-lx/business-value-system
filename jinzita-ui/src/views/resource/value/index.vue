...
<script>
import { listValue, getValue, delValue, addValue, updateValue } from "@/api/resource/value";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除价值榜单编号为"' + ids + '"的数据项？').then(function() {
        return delValue(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('resource/value/export', {
        ...this.queryParams
